...
def main(search_term, num_tweets, api_key):
    ...
    # Check to make sure enough tweets have been pulled to run an adequate analysis
    if len(unique_tweets) < num_tweets/2:
        sys.exit(2)
...
def get_credentials(api_key):
    ...
    try:
        ...
        apikeys                  = credentials.alchemy_apikeys
    except:
...
def search(search_term, num_tweets, auth):
    ...
    while True:
        ...
        if 'errors' in response:
            print "could not find twitter user"
            sys.exit(1)
...
def get_text_sentiment(apikey, tweet, target, output):
    ...
    try:
        ...
        print "[FAILED] Error enriching Tweet (ID %s)" % tweet['id']
        print "Error:", e
...
def dedup(tweets):
    ...
    for tweet in tweets:
        if tweet['id'] not in used_ids:
            used_ids += [tweet['id']]
            collection += [tweet]
    print "[SUCCESSFUL] %d tweets collected after de-duplication" % len(collection)
    return collection
...
def store(tweets):
    ...
    print "[SUCCESSFUL] Tweets stored in MongoDB! Number of documents in twitter_db: %d" % db_count

    return

def makeSentDict(sentDict, score, entities):
    ...
    for user in entities['user_mentions']:
        ...
        if user_name in sentDict:
            sentDict[user_name] +=score
        else:
            sentDict[user_name] = score

    # for url in entities['urls']:
    #     full_url = url['expanded_url']
    #     if full_url in sentDict:
    #         sentDict[full_url] += score
    #     else:
    #         sentDict[full_url] = score
...
def print_results():   
    ...
    sentDict = {}
    tweet_list.append(sentDict)


    for tweet in all_tweets:
        makeSentDict(sentDict, tweet['score'],tweet['entities'])
        tweet_list.append({
            'id':tweet['id'],
    ...
    fileName = tweet_list[1]['screen_name']+'.json'
    with open(os.path.join('sentiment/json_data/',fileName), 'w+') as outfile:
        json.dump(tweet_list, outfile)
        print "[SUCCESSFU] Wrote JSON file"
