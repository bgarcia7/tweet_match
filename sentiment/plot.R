...
sentiment_hist <- sentiment_hist + coord_cartesian(xlim = c(-1, 1));
sentiment_hist + scale_fill_manual(name = "", values = c("red", "#5AA8DF"), labels = c("Negative", "Positive"))
ggsave(filename=paste("twitter_sentiment_raw_",args[1],".png",sep=""))
...
sentiment_dens <- sentiment_dens + coord_cartesian(xlim = c(-1, 1));
sentiment_dens + scale_fill_manual(name = "", values = c("red", "#5AA8DF"), labels = c("Negative", "Positive"))
ggsave(filename=paste("twitter_sentiment_kernel_",args[1],".png",sep=""))
...
time_chart <- time_chart + scale_fill_manual(name = "", values = c("red", "#5AA8DF"), labels = c("Negative", "Positive"))
time_chart + theme(axis.text.x = element_text(angle=45, hjust=1))
ggsave(filename=paste("twitter_sentiment_volume_",args[1],".png",sep=""))
